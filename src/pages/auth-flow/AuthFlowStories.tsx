...
export default function AuthFlowStories({ flowType }: { flowType: FlowType }) {
  ...
  // For org/tenant users: CTA navigates to WelcomeOrgPage (match screen) instead of onboarding.
  const isOrgFlow = Boolean(orgMember || tenantConfig);

  const [steps, setSteps] = useState<StoryStep[]>(() => {
    return flowType === 'new-user' ? newUserSteps : orgUserSteps;
  });
  ...
  const goNext = useCallback(() => {
    if (current >= steps.length - 1) {
      goTo(0, 1); // wrap back to first story
    } else {
      goTo(current + 1);
    }
  ...
  useEffect(() => {
    if (isInteractive || !imagesLoaded) {
      // Interactive slides (e.g. select-org): mark segment as filled (1) — user is here.
  ...
  // One segment per story step — fills as the current story progresses.
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex flex-col">
      ...
      <div className="flex-1 relative overflow-hidden rounded-t-2xl" onClick={handleTap}>
        ...
        {!['how-did-you-arrive', 'select-org'].includes(steps[current]?.id ?? '') && (
          <div className="absolute bottom-0 inset-x-0 z-30 pointer-events-none">
            ...
            <div className="bg-black/70 backdrop-blur-sm px-6 pb-6 pt-1 pointer-events-auto" dir="rtl">
              <div className="flex items-center gap-4">
                ...
                {/* Continue button — org flow navigates to WelcomeOrgPage (match screen);
                    new-user flow skips directly to onboarding */}
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    if (isOrgFlow) {
                      // Navigate to the standalone match screen (WelcomeOrgPage).
                      // setMatchScreenReached(true) is called on WelcomeOrgPage mount.
                  ...
                  style={{
                    ...
                    // Always Nexus purple during stories — org colors only apply
                    // from the match/identification screen onwards.
                    color: '#635bff',
                  }}
                >
